...
# register_page(__name__, path='/views', name='Vues', title='Vues', order=2, icon='carbon:data-view')
...
#     data = [
#         {'value': index, 'label': f'Requête {index + 1}'} for index in range(len(VIEWS))
#     ]
#     return html.Div(
#         [
#             dmc.Select(
...
#             dmc.Table(
#                 children=create_table(execute_sql(*VIEWS[0])),
#                 striped=True,
...
#     sql_query = VIEWS[in_request][0] % VIEWS[in_request][1]
#     table = create_table(execute_sql(*VIEWS[in_request]))
